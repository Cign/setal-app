    ...
    React.useCallback(() => {
      const getPrestationsListByUserId = async () => {
        ...
        try {
          ...
          const { data } = await axios.get(
            `${baseUrl}/api/prestations?populate=*&filters[employe][id][$eqi]=${user?.id}`,
            {
      ...
      const getDashData = async () => {
        ...
        try {
          ...
          // for today: /api/prestations?populate=*&filters[employe][id][$eqi]=1&filters[createdAt][$gte]=2024-02-05T00:00:00Z&filters[createdAt][$lte]=2024-02-06T00:00:00Z #&filters[date_gte]=${today.toISOString()}&filters[date_gte]=${tomorrow.toISOString()}
          const { data } = await axios.get(
            `${baseUrl}/api/prestations/${user?.id}/todaysRecetteAndExpenses`,
            {
              headers: { Authorization: `Bearer ${user?.token}` },
            }
          );
    ...
    >
      <SafeAreaView style={{ ...StyleSheet.absoluteFillObject, flex: 1 }}>
        <ResponseDialog
          ...
          message={"Une nouvelle dépense a été ajoutée !"}
        ...
        <View style={styles.dayInfoContainer}>
          {isTablet ? (
            <>
              <TabCard
                title={"Recettes"}
                number={dashboardData?.todaysTotalRecettes}
                icon={"arrow-down"}
              />
              <TabCard
                title={"Dépenses"}
                number={dashboardData?.todaysTotalDepenses}
                icon={"arrow-up"}
              />
              <TabCard
            ...
            <>
              <View style={styles.boxContainer}>
                ...
                <View style={styles.boxInfoVertical}>
                  ...
                  <Text style={styles.boxInfoVerticalContent}>
                    {dashboardData?.todaysTotalRecettes}
                  </Text>
                </View>
              </View>
              <View style={styles.boxContainer}>
                ...
                <View style={styles.boxInfoVertical}>
                  ...
                  <Text style={styles.boxInfoVerticalContent}>
                    {dashboardData?.todaysTotalDepenses}
                  </Text>
                </View>
              </View>
            </>
        ...
        <View style={[styles.sectionTitle, { marginTop: 16 }]}>
          <Text style={{ color: "darkgrey" }}>
            Liste des Prestations du jour:
          </Text>
          <Text style={{ fontWeight: "bold", marginRight: "7%" }}>
            {" "}
            {!isTablet && countData + "Prestation(s)"}
          </Text>
        </View>
        <FlashList
          data={data}
          ...
          <Sheet.Frame padding="$4" space="$5">
            <YStack>
              ...
              <Input
                ...
                value={objetDepense}
                onChangeText={setObjetDepense}
                style={styles.shadowStyle}
              />
              <Label htmlFor="objet">Montant:</Label>
              <Input
                size="$4"
                id="montant"
