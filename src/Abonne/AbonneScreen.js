...
const AbonneScreen = () => {
  ...
  const contains = ({ attributes: { NomComplet, article, Tel } }, query) => {
    if (
      NomComplet?.toLowerCase().includes(query) ||
      article?.toLowerCase().includes(query) ||
      Tel?.toLowerCase().includes(query)
    ) {
      console.log("hit");
      return true;
    }
    ...
    React.useCallback(() => {
      ...
      const getListPrestasAbonnesOfMonth = async () => {
        try {
          ...
          const { data } = await axios.get(
            `${baseUrl}/api/prestation-abonnes?populate=*&filters[createdAt][$gt]=${startOfMonth}&filters[createdAt][$lte]=${endOfMonth}`,
            {
              headers: { Authorization: `Bearer ${user?.token}` },
            }
          );
          console.log(
            "Successfully got the prestas for ABONNE IN MONTH $$",
            data?.data
          );
      ...
      <SafeAreaView style={{ ...StyleSheet.absoluteFillObject, flex: 1 }}>
          ...
          <View style={[styles.boxContainer, { width: "57%" }]}>
            ...
            <View style={styles.boxInfoVertical}>
              <Text style={styles.boxInfoVerticalTitle}>
                Nombre de prestations abonnés du mois
              </Text>
        ...
        <FlashList
          ...
          renderItem={(item) => (
            <ListCardAbonne
              item={item}
              action={{ clickable: true, destination: "DetailsAbonneScreen" }}
            />
          )}
