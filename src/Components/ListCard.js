...
const SmallCard = ({ item: { item }, action, onPressAction }) => {
  ...
  const ReturnIcon = () => {
    ...
    switch (item?.attributes?.category_lavage?.data?.attributes?.name) {
      case CL.tapis:
        icon = (
          <FontAwesome5 name="scroll" size={24} color={Colors.background} />
        );
        break;
      case CL.voiture:
        icon = (
          <FontAwesome5 name="car-alt" size={24} color={Colors.background} />
        );
        break;
      case CL.moto:
        icon = (
          <FontAwesome5 name="motorcycle" size={24} color={Colors.background} />
        );
        break;
      case CL.local:
        icon = (
          <FontAwesome5 name="building" size={24} color={Colors.background} />
        );
        break;
      default:
        icon = null;
    }
    return  icon ? (<View style={styles.logo}>{icon}</View>) : null;
  };
        ...
        <View style={styles.card}>
          ...
          <View style={styles.mcontainer}>
            ...
            <Text style={styles.coinSymbol}>
              {item?.attributes?.type_lavage?.data?.attributes?.title?.toUpperCase()}
            </Text>
          </View>
